    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            ...
            if "나이키" in filename or "nike" in filename:
                display_items = [p for p in items if p["brand"] == "나이키"]
            elif "푸마" in filename or "puma" in filename:
                display_items = [p for p in items if p["brand"] == "푸마"]
            elif "마뗑킴" in filename or "matin" in filename:
                display_items = [p for p in items if p["brand"] == "마뗑킴"]
            else:
...
def review_create(product_id):
    ...
    return render_template("review_form.html", product=review_product)
