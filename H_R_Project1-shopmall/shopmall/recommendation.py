...
def get_size_recommendation(current_user_id, product_id):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if (
                not current_user.get("height")
                or not current_user.get("weight")
                or not current_user.get("preferred_fit")
            ):
                return {
                    "recommended_size": None,
                    "match_count": 0,
                    "avg_rating": None,
                    "message": "회원 정보에 키, 몸무게, 선호 핏이 충분하지 않습니다.",
                }
            ...
            size_counts = Counter(
                r["purchased_size"] for r in similar_reviews if r.get("purchased_size")
            )
            size_feel_counts = Counter(
                r["size_feel"] for r in similar_reviews if r.get("size_feel")
            )
            fit_feel_counts = Counter(
                r["fit_feel"] for r in similar_reviews if r.get("fit_feel")
            )

            rating_values = [r["rating"] for r in similar_reviews if r.get("rating")]
            ...
            keyword_counts = extract_keywords(
                [r.get("review_text", "") for r in similar_reviews]
            )

            positive = []
            if size_feel_counts:
                positive.append(
                    f"사이즈 체감은 '{size_feel_counts.most_common(1)[0][0]}' 의견이 가장 많아요"
                )
            if fit_feel_counts:
                positive.append(
                    f"핏 체감은 '{fit_feel_counts.most_common(1)[0][0]}' 평가가 많아요"
                )

            summary = (
                f"비슷한 체형 사용자 {len(similar_reviews)}명 중 "
                f"{recommended_size} 사이즈를 가장 많이 선택했어요."
            )
            if avg_rating is not None:
                summary += f" 평균 만족도는 {avg_rating}점이에요."
            if positive:
                summary += " " + " ".join(positive)

            return {
                "recommended_size": recommended_size,
                "recommended_size_count": recommended_size_count,
                "match_count": len(similar_reviews),
